...
class QueryBase {
  ...
  _getDateFilter ({ date } = {}) {
    ...
    if (dateType === 'custom' && date.length > 2 && typeof date[1] === 'string') {
      const from = DateTime.fromISO(date[1])
      const to = date[2] ? DateTime.fromISO(date[2]) : now
    ...
    if (dateType === 'last-hour') {
      const from = now.minus({ hours: 1 })
      const previousPeriodFrom = now.minus({ hours: 2 })
      return { previousPeriodFrom, from, to: now }
    }

    if (dateType === 'last-24h') {
      const from = now.minus({ days: 1 })
      const previousPeriodFrom = now.minus({ days: 2 })
      return { previousPeriodFrom, from, to: now }
    }
