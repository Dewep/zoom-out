...
class Web {
  ...
  async _app () {
    ...
    this._publicRoutes(app)
    this._privateRoutes(app)

    app.use(express.static(path.join(__dirname, '..', '..', '..', '..', '..', 'dist'), {
      setHeaders: (res) => {
        const cspHeaders = (config.csp || {}).headers || {}
        for (const key of Object.keys(cspHeaders)) {
          res.set(key, cspHeaders[key])
        }
      }
    }))
    app.use(express.static(path.join(__dirname, '..', '..', '..', 'dist'), {
    ...
    app.use(function (req, res, next) {
      res.sendFile(path.join(__dirname, '..', '..', '..', '..', '..', 'dist', 'index.html'), { headers: (config.csp || {}).headers || {} }, function (err) {
        if (err) {
          res.sendFile(path.join(__dirname, '..', '..', '..', 'dist', 'index.html'), { headers: (config.csp || {}).headers || {} }, function (err2) {
            if (err2) {
              next(err2)
            }
          })
