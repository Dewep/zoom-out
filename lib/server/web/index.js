...
class Web {
    ...
    wsServer.on('connection', (wsConnection, req) => {
      ...
      wsConnection.on('message', async (message) => {
        try {
          ...
          if (type === 'publish') {
            await privateCtrl.publish(data)
            return wsConnection.send(JSON.stringify({ type: 'published', data: {} }))
          }
  ...
  _auth (type) {
    return this._async(async function (req) {
      ...
      if (type === 'private' && req.headers.authorization && config.apiKeys.includes(req.headers.authorization)) {
        return
      }
      ...
      if (type === 'public' && req.headers.authorization) {
        try {
          req.user = new User(await jwt.verify(req.headers.authorization))
          return
        } catch (err) {}
      }
