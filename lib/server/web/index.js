...
class Web {
  ...
  async _app () {
    ...
    app.use(express.static(path.join(__dirname, '..', '..', '..', '..', '..', 'dist'), {
      setHeaders: (res) => {
        const cspHeaders = this._getCSPHeaders
    ...
    app.use(function (req, res, next) {
      res.sendFile(path.join(__dirname, '..', '..', '..', '..', '..', 'dist', 'index.html'), { headers: this._getCSPHeaders }, function (err) {
        if (err) {
          res.sendFile(path.join(__dirname, '..', '..', '..', 'dist', 'index.html'), { headers: this._getCSPHeaders }, function (err2) {
            if (err2) {
              next(err2)
            }
          })
