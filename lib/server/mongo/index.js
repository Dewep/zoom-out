...
class Mongo {
  async client () {
    if (!this._clientPromise) {
      const url = config.mongo.url || `mongodb://${config.mongo.host}:${config.mongo.port}/${config.mongo.database}`
      const client = new MongoClient(url)
      this._clientPromise = await client.connect()
    }
  ...
  async pagination (collectionName, filters, options = {}) {
    const page = options.page || 1
    const projection = options.projection || {}
    const nbPerPage = options.limit || 100
    const sort = options.sort || {}
    const maxNextPage = options.maxNextPage || 10

    return this.custom(collectionName, async (collection) => {
      const skip = nbPerPage * (page - 1)
      const cursor = collection
        .find(filters, { projection })
        .sort(sort)
        .skip(skip)

      const remaining = await cursor.limit(maxNextPage * nbPerPage).count()
      const results = await cursor.limit(nbPerPage).toArray()

      const total = skip + remaining
      const nbPages = Math.ceil(total / nbPerPage)

      return { results, page, nbPerPage, total, nbPages }
    })
  }
  ...
  async insertOne (collectionName, doc, options) {
    return this.custom(collectionName, collection => {
      return collection.insertOne(doc, options)
        .then(result => ({ ...doc, _id: result.insertedId }))
    })
  }

  async insertOrUpdate (collectionName, query, values) {
    return this.updateOne(collectionName, query, values, { upsert: true })
  }
  ...
  async updateMany (collectionName, filter, update, options) {
    return this.custom(collectionName, collection => {
      return collection.updateMany(filter, update, options)
        .then(result => ({ modifiedCount: result.modifiedCount, upsertedCount: result.upsertedCount, matchedCount: result.matchedCount }))
    })
  }
